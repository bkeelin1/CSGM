#' @title Summarize Results for CSGM Package functions
#'
#' @description A collection of S3 methods used to extract and summarize the results from
#' regression based functions within the CSGM package.
#'
#' @param object An object resulting from functions within the package.
...
#' @export
summary.reg.bio <- function(object, ...) {
...
summary.GMA <- function(object, ...) {
  ...
  print(object$morphotree$resid$k_medoid_plot)
  print("Plot 2: Principal Component Plot for PCs 1-3")
  print(object$PCA$PCA_plots$PCs_1_2_3)
  print("Plot 3: Shape variation across PC 1")
  print(object$PCA$lollipop_PCs[[1]])
  print("Plot 4: Shape variation across PC 2")
  print(object$PCA$lollipop_PCs[[2]])
  cat("\n")
}


#' @export
summary.AABA <- function(object, ...) {
      ...
      for(i in 1:i_length) {
        ...
        if("pred.subset" %in% names(object_pred[[m]])) {
          ...
          for(j in 1:j_length) {
            ...
            pred_Output[[m]][[i]][j+1,] <- pred.subset # NOTE: there was a typo here in original `pred.subset` vs `pred_subset`. I standardized to pred.subset.
  ...
  for(m in 1:m_length) {
    ...
    for(i in 1:i_length) {
      if(!is.null(Cor_Output) && !is.null(Cor_Output[[m]][[i]])) {
        ...
        combined_df <- data.frame(row.names = rownames(PSLR_Output[[m]][[i]])) # FIXED typo: pslr_Output to PSLR_Output
...
#' @export
summary.cor.bio <- function(object, ...) {
...
#' @export
summary.pls.bio <- function(object, ...) {
...
#' @export
summary.pslr.bio <- function(object, ...) {
