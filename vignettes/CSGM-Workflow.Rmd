...
vignette: >
  %\VignetteIndexEntry{CSGM-Workflow}
...
# CSGM – Cross-Sectional Geometric Morphometrics
...
curves_new <- gen_curves(ncurves = 10, # 10 curves
                         nsemiland = 590, # total semiland
                         def_semi = semiland_indices, 
                         def_points = anchor_indices, 
                         closed = FALSE # no closed outlines
                         )
```
...
# Skeletal Collection
Group = list(Collection = Corpus_Prop[1:50,]$Collection) 

# Conduct the analysis
output <- GMA(Corpus_Land,  
              k = 3,  # 3D data
              ...
              subset = paste(rep(c("LM1M2", 
                                   ...
                                   "RP3P4", 
                                   "RM1M2"), each = 118)),
              bilat_sym = TRUE,  #bilateral symmmetry test
              # left and right sides
              land.pairs = cbind(left = c(1:236), 
                                 right = c(355:590)), 
              object.sym = TRUE, # object is symmetric
              ind = ID, # needed for bilateral symmetry
              w_morpho_pca = TRUE, # Dist weighted PCA
...
```{r PCA_Plots,echo=TRUE,eval=FALSE}
...
### SCG: Visualize Morphological shape changes by Collection
...
# View the Group Differences by collection (there are little)

Group_shapes[[1]]

```

### SCG: Vizualize Morphological shape changes by overall bending/torsional resistance

```{r SCG,echo=TRUE,eval=FALSE}

left_resistance = Corpus_Prop %>% 
  filter(Region == c("LM1M2","LP3P4")) %>%
  group_by(ID) %>% 
  reframe(across(where(is.numeric), mean, na.rm = TRUE))

right_resistance = Corpus_Prop %>% 
  filter(Region == c("RM1M2","RP3P4")) %>%
  group_by(ID) %>% 
  reframe(across(where(is.numeric), mean, na.rm = TRUE))

SCG_Groups = list(Left_Resistance = left_resistance$J,
                  Right_Resistance = right_resistance$J)

Group_shapes = SCG(output$GPA$coords,
                   Groups = SCG_Groups,
                   coord_name = dimnames(Corpus_Land)[[1]],
                   subset = paste(rep(c("LM1M2", "LP3P4",
                                        "Symphysis", 
...
# View Left corpus shape variation by Polar second moment of area (J)
Group_shapes[[1]]
# View Left corpus shape variation by Polar second moment of area (J)
Group_shapes[[2]]
```
...
![Figure 2. Workflow for this hypothesis test that the posterior mandibular corpus is biomechanically integrated with the symphysis.](images/CSGM_Models.png)
...
cor_hypothesize2 = cor.bio(Models, #Your hypothesis models
                          point_set, # Your subsets
                          paired_subsets = FALSE #Not every subset combination
                          )
```

Notes: Visualize the results of the correlograms and correlation networks in your directory under the folder "Correlations_Bio".

#### Obtain summary statistics for cor.bio function

```{r Correlation_Summary,echo=TRUE,eval=FALSE}
summary(cor_hypothesize)
```
...
```{r MARS_Reg_Summary,echo=TRUE,eval=FALSE}
# Hypothesis test 1
summary(pred_tests[[1]])

# Hypothesis test 2
summary(pred_tests[[2]])

# Hypothesis test 3
summary(pred_tests[[3]])

# Hypothesis test 4
summary(pred_tests[[4]])
```
